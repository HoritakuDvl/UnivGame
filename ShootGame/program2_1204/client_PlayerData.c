...
//プレイヤー(pos:各クライアントの番号)
    ...
    if(player[pos].flag > 0){
        ...
        if(cnt <= 4 || 11 <= cnt) {
            ...
            //砲台
                SDL_Rect dst_2;
                dst_2 = DstRectInit(player[pos].dst.x, player[pos].dst.y);
                SDL_BlitSurface(gPlayer2_2, &player[pos].src, window, &dst_2);
            ...
            for(t = 0; t < PLAYER_ORDER_MAX; t++){
                if(player[i].knd == playerOrder[t].knd){
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        SrcRectInit(0, 0, playerOrder[t].w2, playerOrder[t].h2);
